-- Position-level outcomes, for the "Are users making money?" redesign.
--
-- Reframes the panel away from WALLETS and toward MARKETS + POSITIONS. One
-- trader's stake in one market side = one position (the (wallet, belief, side)
-- grain we already build in `pos`). Every position lands in exactly one of
-- three mutually-exclusive states, so the counts always reconcile:
--
--   * won_positions  — closed (sold out) and got back MORE than put in (real)
--   * lost_positions — closed and got back LESS than put in (real)
--   * open_positions — still holding, not settled yet (paper);
--                      split into open_up / open_down at the last trade price.
--
-- "Closed" uses a tiny relative dust threshold (<= 0.01% of tokens ever bought
-- still held) so rounding-sized leftovers still count as fully exited. A
-- partly-sold-but-still-open position counts as `open` — its outcome isn't
-- final until it's fully closed.
--
-- This is ADDITIVE: it keeps every column from 20260711180000 (the wallet
-- ledger + concentration) and appends 5 position columns. Return shape changes
-- (26 -> 31 cols), so the function is dropped first. Executable by anon (the
-- /api/public routes call with the publishable key).
...
RETURNS TABLE(
  ...
  net_eth numeric,
  net_usd numeric,
  -- wallet ledger: every trader in one bucket by combined total P&L
  wallets_total int,
  ahead int,        -- banked + paper_up
  behind int,       -- locked_loss + underwater
  banked int,       -- cashed out, up (real)
  paper_up int,     -- still holding, up (paper)
  underwater int,   -- still holding, down (paper)
  locked_loss int,  -- cashed out, down (real)
  -- concentration of realized (real) profit
  top3_gain_share numeric,
  top5_gain_share numeric,
  -- position ledger: every (wallet, belief, side) position in one state
  won_positions int,   -- closed, came out ahead (real)
  lost_positions int,  -- closed, came out behind (real)
  open_positions int,  -- still open, not settled (paper)
  open_up int,         -- open and up at last trade price (paper)
  open_down int        -- open and down at last trade price (paper)
)
LANGUAGE plpgsql
...
BEGIN
  ...
  -- combined per-wallet score (realized + unrealized), in wei
  scored AS (
    SELECT pw.*,
  ...
  -- realized-profit winners, ranked per snapshot (real money extracted)
  gains AS (
  ...
  -- position ledger: each (wallet, belief, side) position in one state.
  -- closed = <= 0.01% of tokens ever bought still held (dust-tolerant).
  posstat AS (
    SELECT p.lbl,
      COUNT(*) FILTER (
