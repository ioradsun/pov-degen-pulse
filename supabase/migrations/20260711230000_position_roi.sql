-- Capital-weighted Avg ROI % per position state, for the "Are users making
-- money?" panel. Splits the display into a 2x2 — settled vs. paper × profit vs.
-- loss — each cell carrying a count and an average return.
--
-- ROI is capital-weighted (SUM(pnl) / SUM(cost)), i.e. "what the average ETH in
-- this bucket returned", so a tiny dust position that 10x'd can't hijack the
-- number. Returned as a dimensionless fraction (0.40 = +40%); the UI scales it.
--   * won_roi       — realized ÷ cash in, over closed winners
--   * lost_roi      — realized ÷ cash in, over closed losers   (negative)
--   * open_up_roi   — unrealized ÷ cost still in, over open winners
--   * open_down_roi — unrealized ÷ cost still in, over open losers (negative)
--
-- Additive: keeps every existing column, appends 4. Return shape changes
-- (31 -> 35 cols), so the function is dropped first. Executable by anon.
...
RETURNS TABLE(
  ...
  open_up int,
  open_down int,
  -- capital-weighted average ROI per position state (fraction; 0.40 = +40%)
  won_roi numeric,
  lost_roi numeric,
  open_up_roi numeric,
  open_down_roi numeric
)
LANGUAGE plpgsql
...
BEGIN
  ...
  -- position ledger + capital-weighted ROI. `cl` = closed (dust-tolerant),
  -- `op` = open; `ppnl` = paper (unrealized) P&L on the open portion.
  pflags AS (
    SELECT p.*,
      (p.remaining_tk <= p.bought_tk * 1e-4 AND p.sell_wei > 0) AS cl,
      (p.remaining_tk >  p.bought_tk * 1e-4)                    AS op,
      (GREATEST(p.remaining_tk,0) * COALESCE(p.px_wei_per_tkwei,0)
        - GREATEST(p.remaining_cost_wei,0))                     AS ppnl
    FROM pos p
  ),
  posstat AS (
    SELECT f.lbl,
      COUNT(*) FILTER (WHERE f.cl AND f.r_eth_wei > 0)::int  AS won_positions,
      COUNT(*) FILTER (WHERE f.cl AND f.r_eth_wei <= 0)::int AS lost_positions,
      COUNT(*) FILTER (WHERE f.op)::int                      AS open_positions,
      COUNT(*) FILTER (WHERE f.op AND f.ppnl > 0)::int       AS open_up,
      COUNT(*) FILTER (WHERE f.op AND f.ppnl <= 0)::int      AS open_down,
      SUM(f.r_eth_wei) FILTER (WHERE f.cl AND f.r_eth_wei > 0)
        / NULLIF(SUM(f.buy_wei) FILTER (WHERE f.cl AND f.r_eth_wei > 0),0)   AS won_roi,
      SUM(f.r_eth_wei) FILTER (WHERE f.cl AND f.r_eth_wei <= 0)
        / NULLIF(SUM(f.buy_wei) FILTER (WHERE f.cl AND f.r_eth_wei <= 0),0)  AS lost_roi,
      SUM(f.ppnl) FILTER (WHERE f.op AND f.ppnl > 0)
        / NULLIF(SUM(GREATEST(f.remaining_cost_wei,0)) FILTER (WHERE f.op AND f.ppnl > 0),0)  AS open_up_roi,
      SUM(f.ppnl) FILTER (WHERE f.op AND f.ppnl <= 0)
        / NULLIF(SUM(GREATEST(f.remaining_cost_wei,0)) FILTER (WHERE f.op AND f.ppnl <= 0),0) AS open_down_roi
    FROM pflags f
    GROUP BY f.lbl
  )
  SELECT
    ...
    MAX(posstat.won_roi)        AS won_roi,
    MAX(posstat.lost_roi)       AS lost_roi,
    MAX(posstat.open_up_roi)    AS open_up_roi,
    MAX(posstat.open_down_roi)  AS open_down_roi
  FROM scored s
