...
BEGIN
  ...
  SELECT
    pw.lbl AS label,
    COUNT(*) FILTER (WHERE pw.out_wei > 0)::int AS sellers,
    COUNT(*) FILTER (WHERE pw.out_wei > 0 AND pw.realized_eth_wei > 0)::int AS realized_winners,
    (COALESCE(SUM(pw.realized_eth_wei) FILTER (WHERE pw.out_wei > 0),0) / 1e18)::numeric AS realized_net_eth,
    (COALESCE(SUM(pw.realized_usd)     FILTER (WHERE pw.out_wei > 0),0))::numeric        AS realized_net_usd,
    COUNT(*) FILTER (WHERE pw.rem_tk_total > 0)::int AS holders,
    COUNT(*) FILTER (WHERE pw.rem_tk_total > 0 AND (pw.hold_wei - pw.rem_cost_wei) > 0)::int AS holder_winners,
    (COALESCE(SUM(pw.hold_wei - pw.rem_cost_wei) FILTER (WHERE pw.rem_tk_total > 0),0) / 1e18)::numeric AS unrealized_eth,
    (COALESCE(SUM(pw.hold_usd - pw.rem_cost_usd) FILTER (WHERE pw.rem_tk_total > 0),0))::numeric        AS unrealized_usd,
    (COALESCE(SUM(pw.hold_wei) FILTER (WHERE pw.rem_tk_total > 0),0) / 1e18)::numeric AS holding_value_eth,
    (COALESCE(SUM(pw.hold_usd) FILTER (WHERE pw.rem_tk_total > 0),0))::numeric        AS holding_value_usd,
    (COALESCE(SUM(pw.in_wei),0)  / 1e18)::numeric AS money_in_eth,
    (COALESCE(SUM(pw.in_usd),0))::numeric         AS money_in_usd,
    (COALESCE(SUM(pw.out_wei),0) / 1e18)::numeric AS money_out_eth,
    (COALESCE(SUM(pw.out_usd),0))::numeric        AS money_out_usd,
    ((COALESCE(SUM(pw.out_wei),0) + COALESCE(SUM(pw.hold_wei) FILTER (WHERE pw.rem_tk_total > 0),0) - COALESCE(SUM(pw.in_wei),0)) / 1e18)::numeric AS net_eth,
    ( COALESCE(SUM(pw.out_usd),0) + COALESCE(SUM(pw.hold_usd) FILTER (WHERE pw.rem_tk_total > 0),0) - COALESCE(SUM(pw.in_usd),0))::numeric        AS net_usd
  FROM per_wallet pw
  GROUP BY pw.lbl;
END; $$;

REVOKE EXECUTE ON FUNCTION public.trader_outcomes(text) FROM PUBLIC, anon, authenticated;
