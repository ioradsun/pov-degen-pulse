-- Per-wallet position breakdown for the wallet lookup page.
--
-- Same position engine as trader_outcomes() (per_key → ev → last_price → pos),
-- but scoped to ONE wallet and NOT aggregated: it returns one row per
-- (belief, side) the wallet ever touched, joined to the belief's title/slug.
-- Lifetime (no range) and priced at the most recent trade for each market side.
--
-- Address is matched case-insensitively — callers should pass a 0x address; we
-- lower() both sides so casing can't silently drop a wallet's history.
--
-- roi is a fraction (0.40 = +40%): closed positions use realized ÷ cash in,
-- open positions use unrealized ÷ cost still at risk. Executable by anon.
