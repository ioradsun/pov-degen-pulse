CREATE OR REPLACE FUNCTION public.activity_buckets(granularity text DEFAULT 'hour', buckets_back integer DEFAULT 24)
...
BEGIN
  ...
  RETURN QUERY
  WITH series AS (
    SELECT date_trunc(granularity, NOW()) - (n * step) AS b
  ...
  trade_agg AS (
    SELECT
      date_trunc(granularity, block_timestamp) AS b,
  ...
  belief_agg AS (
    SELECT date_trunc(granularity, created_at) AS b, COUNT(*)::int AS created
