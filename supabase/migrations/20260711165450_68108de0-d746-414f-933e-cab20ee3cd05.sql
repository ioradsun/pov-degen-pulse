...
BEGIN
  ...
  scored AS (
    SELECT
      pw.*,
      (pw.out_wei + pw.hold_wei - pw.in_wei) AS score_wei,
      (pw.rem_tk_total > 0) AS has_holdings
    FROM per_wallet pw
  ),
  top_gains AS (
    SELECT lbl,
      COALESCE(SUM(realized_eth_wei) FILTER (WHERE realized_eth_wei > 0), 0) AS total_pos,
      COALESCE((
        SELECT SUM(t3.realized_eth_wei) FROM (
          SELECT s2.realized_eth_wei
          FROM scored s2
          WHERE s2.lbl = scored.lbl AND s2.realized_eth_wei > 0
          ORDER BY s2.realized_eth_wei DESC
          LIMIT 3
        ) t3
      ), 0) AS top3_pos
    FROM scored
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE s.score_wei > 0)::int AS ahead,
    COUNT(*) FILTER (WHERE s.score_wei < 0)::int AS behind,
    COUNT(*) FILTER (WHERE s.score_wei > 0 AND NOT s.has_holdings)::int AS banked,
    COUNT(*) FILTER (WHERE s.score_wei > 0 AND s.has_holdings)::int AS paper_up,
    COUNT(*) FILTER (WHERE s.score_wei < 0 AND s.has_holdings)::int AS underwater,
    COUNT(*) FILTER (WHERE s.score_wei < 0 AND NOT s.has_holdings)::int AS locked_loss,
    CASE WHEN MAX(tg.total_pos) > 0 THEN (MAX(tg.top3_pos) / MAX(tg.total_pos))::numeric ELSE NULL END AS top3_gain_share
  FROM scored s
  LEFT JOIN top_gains tg ON tg.lbl = s.lbl
  GROUP BY s.lbl;
END; $function$;

REVOKE ALL ON FUNCTION public.trader_outcomes(text) FROM PUBLIC, anon, authenticated;
