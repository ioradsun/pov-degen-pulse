...
ALTER TABLE public.belief_stats ADD COLUMN IF NOT EXISTS market_cap_usd NUMERIC NOT NULL DEFAULT 0;
...
BEGIN
  ...
  SELECT
    ...
    GREATEST(
      COALESCE(SUM(t.gross_amount_usd) FILTER (WHERE t.action='buy'), 0)
      - COALESCE(SUM(t.gross_amount_usd) FILTER (WHERE t.action='sell'), 0),
      0
    )
...
-- Backfill market cap immediately from all-time trades (30d window is what refresh uses)
UPDATE public.belief_stats s SET market_cap_usd = GREATEST(sub.mc, 0)
FROM (
  SELECT b.belief_id,
    COALESCE(SUM(t.gross_amount_usd) FILTER (WHERE t.action='buy'), 0)
    - COALESCE(SUM(t.gross_amount_usd) FILTER (WHERE t.action='sell'), 0) AS mc
  FROM public.beliefs b
  LEFT JOIN public.trades t ON t.belief_id=b.belief_id AND t.is_canonical=TRUE
  GROUP BY b.belief_id
) sub
WHERE s.belief_id = sub.belief_id;
