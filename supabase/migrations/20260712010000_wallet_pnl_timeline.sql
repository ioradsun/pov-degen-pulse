...
--
-- Design (see also 20260712000000_wallet_positions.sql for the position math):
--   * wallet_watch          — every wallet someone searched (registered on view)
--   * wallet_pnl_snapshot   — one row per (wallet, day); PK gives idempotent upsert
--   * wallet_snapshot_now() — current rollup (aggregates wallet_positions)
--   * wallet_backfill_snapshots() — reconstruct the full past from trades on first
--                             search (realized exact; holdings marked at the price
--                             prevailing at each day's close — an ESTIMATE)
--   * wallet_register()     — upsert watch + backfill-if-new + refresh today
--   * snapshot_watched_wallets() — pg_cron daily append for active wallets
--   * wallet_timeline()     — read the series for the chart
--
-- Tables have RLS on with no policies: all access is via SECURITY DEFINER
-- functions (owned by the migration role, which bypasses RLS), so the anon key
-- never touches the tables directly. Addresses are stored lowercased.
...
-- ---------- register on search: watch + backfill-if-new + today ----------
...
-- ---------- daily cron: snapshot recently-active watched wallets ----------
