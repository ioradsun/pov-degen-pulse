...
AS $function$
  ...
  bounds AS (
    SELECT
      win,
      NOW() - win     AS cur_start,
      NOW() - 2 * win AS prev_start,
      NOW() - win     AS prev_end
    FROM b
  ),
  buys AS (
  ...
  SELECT
    ...
    CASE WHEN (SELECT win FROM bounds) IS NULL THEN NULL ELSE COALESCE(buys_prev.vol, 0) END,
    ...
    CASE WHEN (SELECT win FROM bounds) IS NULL OR buys_prev.vol IS NULL
