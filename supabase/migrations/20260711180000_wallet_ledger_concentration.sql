-- Wallet ledger + concentration, for the "Are users making money?" redesign.
--
-- Builds on the set-based trader_outcomes() from 20260711163220 (which derives
-- each wallet's remaining cost basis as total-buy-cost minus the FIFO cost
-- already consumed by sells, reusing realized_pnl_events_cache — no second
-- replay). This migration keeps that implementation and ADDS:
--   * per-wallet ledger buckets — every trader in exactly one bucket by
--     combined score (cash out + shares still held, marked at last price,
--     minus money in): ahead/behind, and the four states banked / paper_up /
--     underwater / locked_loss.
--   * concentration of REAL (realized) profit — share held by the top 3 / 5
--     winning wallets.
--
-- Return shape changes (17 -> 26 cols), so the function is dropped first.
-- Executable by anon (the /api/public routes call with the publishable key).

DROP FUNCTION IF EXISTS public.trader_outcomes(text);
DROP FUNCTION IF EXISTS public._outcomes_at(timestamptz);
